...
| P14 | `-V` / `--version` (client-local UX) | **DONE** | `-V`/`--version` prints version, negotiable protocol range (`RsyncConstants.ProtocolVersion`/`MinProtocolVersion`, never a literal), the checksum offer (`ChecksumNegotiator.DefaultOffer`), and the scoped-out list; stdout, exit 0. **Zero wire bytes**: `ServerArgvBuilder` untouched, so no capture/interop gate (P12/P13-style). Version read from `AssemblyInformationalVersionAttribute` (`VersionInfo`) with the SourceLink `+sha` stripped — git tags stay the single source of truth; verified that CI's `-p:Version=` reaches the line and that a `TrimMode=full` build still resolves it. Lowercase `-v` stays an explicit "unsupported option" — reserved for `--verbose` — and is pinned by test |
...
display (P13) and `-V`/`--version` (P14) — both client-local, no wire. The live interop matrix verifies stock rsync 3.4.3 and 3.4.4 peers through Smoke,
