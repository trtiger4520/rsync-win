...
| 大於 2 GiB 的**推送來源**檔案 | 送出端比對時整檔讀入記憶體，尚未串流化（拉取端為串流） |
| `--delete-after`/`--delete-delay` 的 del-stats varint 區塊 | 我們從不送這些旗標，其線路格式未擷取 |

P10 曾有一項**記憶體取捨**：`-z` 拉取的連續解壓會把該檔字面內容全部緩存在記憶體，且以前綴解壓定位每個
run 的邊界，時間為 run 數的二次方。已改為單一連續 `RunInflater` 邊收邊解、直接寫入目的檔——記憶體與檔案
大小無關，時間線性（實測 64→256 MiB 為 3.2 倍）。
