...
- `.`: the transfer root is sent as literal name `.` with `XMIT_TOP_DIR`. Every source-arg dir gets TOP_DIR; under `-r` all sent dirs have content, so `XMIT_NO_CONTENT_DIR` does not appear; the combination TOP_DIR|NO_CONTENT_DIR marks an *implied* dir (e.g. `--relative` ancestors). Only a **directory** source arg produces `.`. A **single-file** source arg (`rsync file host::mod/`) sends exactly one entry named by the file's basename with **no** `.` and **no** TOP_DIR (MEASURED in `ssh31-push-delta`/`ssh31-push-redo`: single entry, xflags `0x18` = `SAME_UID|SAME_GID`, name `b003_300k.bin`/`big.bin`, mode `0o100644`). The push-side `FileEnumerator` mirrors this: a file root yields one basename entry, a directory root the `.` + descendants.
